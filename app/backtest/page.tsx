...
// ─── /backtest ──────────────────────────────────────────────────────────────
// Investor-grade view of the DEPLOYED model's out-of-sample performance. Data is
// generated by zentory_algo/build_backtest_json.py from research.ensemble_signal
// (step=0) — the exact continuous ensemble the live vault runs, parity-verified
// vs zentory-engine. No fitting in the browser. Honesty is the product: the edge
// is drawdown reduction, NOT beating the asset on every entry (see the cohort section).
...
type CohortH = {
  days: number; beatAll: number; medAll: number;
  rallyBeat: number; rallyMed: number; crashBeat: number; crashMed: number;
};
type AssetData = {
  oosStart: string; oosEnd: string; nFolds: number; frozenSpec: string;
  avgExposure: number; drawdownWins: number; sortinoWins: number;
  sats: SatsBlock;
  cohort: { horizons: CohortH[]; note: string };
...
export default function BacktestPage() {
  ...
  const c90 = useMemo(() => a?.cohort?.horizons.find((h) => h.days === 90) ?? null, [a]);
  const c180 = useMemo(() => a?.cohort?.horizons.find((h) => h.days === 180) ?? null, [a]);
  const cohortBars = useMemo(() => (c90 ? [
    { k: "Any day", beat: c90.beatAll, fill: GRAY },
    { k: "Dip → rally", beat: c90.rallyBeat, fill: RED },
    { k: "Before a crash", beat: c90.crashBeat, fill: "#34d399" },
  ] : []), [c90]);
...
<p className="text-sm leading-relaxed" style={{ color: "rgba(234,234,234,0.75)" }}>
          On <strong style={{ color: TEXT }}>absolute return</strong> the full-cycle picture is honest: out-of-sample,
          BTC, ETH and SOL <em>beat</em> holding while XRP roughly <strong style={{ color: TEXT }}>matches</strong> it —
          and all four cut max drawdown sharply. But that is the full ride. No leverage, no shorts — so in a straight
          parabolic run simply holding wins, and (see the entry-timing section below) a deposit timed into a dip right
          before a rally usually lags holding until the trend re-establishes. We show every asset and every entry.
        </p>
      </section>
      ...
      {a && (
        <>
          ...
          {/* Entry-timing cohort — the honest answer to "what if I deposit at the wrong moment?" */}
          {c90 && (
            <section className="space-y-3 rounded-2xl border p-6" style={{ borderColor: `${GOLD}40`, background: `${GOLD}0a` }}>
              <h3 className="text-lg font-semibold" style={{ color: TEXT }}>If you deposit on a random day — what actually happens?</h3>
              <p className="text-sm leading-relaxed" style={{ color: "rgba(234,234,234,0.7)" }}>
                The full-cycle numbers above hide what matters to a real depositor: your result depends on <em>when</em> you
                enter. Across <strong style={{ color: TEXT }}>every possible deposit day</strong>, here is how often a share
                beat simply holding {asset} over the next 90 days — split honestly between depositing into a dip that then
                rallies (the worst case for a strategy that steps to cash) and depositing before a crash (where the
                protection pays off).
              </p>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <div className="text-[11px] uppercase tracking-wider mb-2" style={{ color: "rgba(234,234,234,0.45)" }}>Any random day · 90d</div>
                  <div className="text-2xl font-bold tabular-nums" style={{ color: TEXT }}>{fmtPct(c90.beatAll, 0)}</div>
                  <div className="text-[10px] mt-1" style={{ color: "rgba(234,234,234,0.4)" }}>of entries beat holding · typical {fmtPct(c90.medAll, 0)}</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <div className="text-[11px] uppercase tracking-wider mb-2" style={{ color: "rgba(234,234,234,0.45)" }}>Bought a dip → rally · 90d</div>
                  <div className="text-2xl font-bold tabular-nums" style={{ color: RED }}>{fmtPct(c90.rallyBeat, 0)}</div>
                  <div className="text-[10px] mt-1" style={{ color: "rgba(234,234,234,0.4)" }}>beat holding · typically {fmtPct(c90.rallyMed, 0)} vs just holding</div>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-5">
                  <div className="text-[11px] uppercase tracking-wider mb-2" style={{ color: "rgba(234,234,234,0.45)" }}>Deposited before a crash · 90d</div>
                  <div className="text-2xl font-bold tabular-nums" style={{ color: "#34d399" }}>{fmtPct(c90.crashBeat, 0)}</div>
                  <div className="text-[10px] mt-1" style={{ color: "rgba(234,234,234,0.4)" }}>beat holding · typically {fmtPct(c90.crashMed, 0)} ahead</div>
                </div>
              </div>
              <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4">
                <div className="text-xs mb-2" style={{ color: "rgba(234,234,234,0.5)" }}>% of deposits that beat holding {asset} over 90 days, by entry timing</div>
                <ResponsiveContainer width="100%" height={220}>
                  <BarChart data={cohortBars} margin={{ top: 8, right: 16, bottom: 0, left: 0 }}>
                    <CartesianGrid stroke="rgba(255,255,255,0.06)" vertical={false} />
                    <XAxis dataKey="k" tick={{ fill: "rgba(234,234,234,0.55)", fontSize: 12 }} />
                    <YAxis domain={[0, 1]} tickFormatter={(v) => `${(v * 100).toFixed(0)}%`} tick={{ fill: "rgba(234,234,234,0.4)", fontSize: 11 }} width={44} />
                    <Tooltip
                      cursor={{ fill: "rgba(255,255,255,0.04)" }}
                      contentStyle={{ background: "#12141a", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 12, color: TEXT }}
                      formatter={((v: unknown) => [fmtPct(Number(v), 0), "Beat holding"]) as never}
                    />
                    <ReferenceLine y={0.5} stroke={GRAY} strokeDasharray="4 4" label={{ value: "coin flip", fill: "rgba(234,234,234,0.45)", fontSize: 10, position: "insideTopRight" }} />
                    <Bar dataKey="beat" radius={[4, 4, 0, 0]} isAnimationActive={false}>
                      {cohortBars.map((b) => <Cell key={b.k} fill={b.fill} />)}
                    </Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>
                  ...
                  <table className="w-full min-w-[520px] text-xs tabular-nums">
                    <thead>
                      <tr className="border-b border-white/10 text-left" style={{ color: "rgba(234,234,234,0.5)" }}>
                        {["Entry timing", "Beat holding (90d)", "Typical (90d)", "Beat holding (180d)"].map((h) => (
                    ...
                    <tbody style={{ color: "rgba(234,234,234,0.75)" }}>
                      {[
                        ["Any random day", c90.beatAll, c90.medAll, c180.beatAll],
                        ["Bought a dip before a rally", c90.rallyBeat, c90.rallyMed, c180.rallyBeat],
                        ["Deposited before a crash", c90.crashBeat, c90.crashMed, c180.crashBeat],
                      ].map(([label, b90, m90, b180]) => (
                        <tr key={label as string} className="border-b border-white/5">
                          <td className="px-4 py-2.5">{label}</td>
                          <td className="px-4 py-2.5">{fmtPct(b90 as number, 0)}</td>
                          <td className="px-4 py-2.5" style={{ color: (m90 as number) >= 0 ? "#34d399" : RED }}>{fmtPct(m90 as number, 0)}</td>
                          <td className="px-4 py-2.5">{fmtPct(b180 as number, 0)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              <p className="text-sm leading-relaxed" style={{ color: "rgba(234,234,234,0.7)" }}>
                <strong style={{ color: TEXT }}>The honest takeaway:</strong> this vault is insurance against the deep
                drawdowns, not a way to beat {asset} on every entry. Deposit into a dip expecting to out-run a rally and
                you will usually lag holding; deposit before a downturn and you are almost always protected. The edge is
                real but it compounds over a <em>full cycle</em> — short, well-timed bull entries are exactly where a
                long/flat strategy gives ground. We would rather you know that before you deposit.
              </p>
            </section>
          )}
...
<li><strong style={{ color: TEXT }}>Deployed model — no fitted parameters:</strong> exposure = the mean long/flat vote across the whole trend+vol-gate family (continuous 0–100%) — the exact model the live vault runs, parity-verified against the engine. Nothing is selected, so there is nothing to over-fit. We deliberately do <em>not</em> hyper-optimize — that&apos;s how backtests lie.</li>
