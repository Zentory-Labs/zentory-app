...
export function TubesBackground({
  ...
  return (
    <div 
      className={cn("relative w-full h-full min-h-[400px] overflow-hidden bg-[#0a0a0a]", className)}
      onClick={handleClick}
    >
      <canvas 
        ref={canvasRef} 
      ...
      {/* Content Overlay — pointer-events-none so canvas gets mouse; children can set pointer-events-auto for links */}
      <div className="relative z-10 w-full h-full min-h-full pointer-events-none [&>*]:pointer-events-auto">
        {children}
      </div>
    </div>
  );
